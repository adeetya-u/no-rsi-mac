...
final class BreakTracker: NSObject, ObservableObject {
    ...
    private func setupNSEventMonitors() {
        // Remove old monitors if they exist
        ...
        globalMonitor = NSEvent.addGlobalMonitorForEvents(matching: .keyDown) { [unowned self] event in
            self.handleKeystroke(source: "NSEvent-Global")
        }
        
        // Local monitor (this app)
        localMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [unowned self] event in
            self.handleKeystroke(source: "NSEvent-Local")
            return event
        }
    }
    ...
    private func handleKeystroke(source: String) {
        ...
        // Terminal debug for count stuck at 0
        NSLog("[DEBUG] Key from \(source) | mode=\(mode) | isOnBreak=\(isOnBreak) | count=\(keystrokeCount)")
        
        guard mode == .keystrokes, !isOnBreak else {
            NSLog("[DEBUG] BLOCKED - wrong mode or on break")
            return
        }
        
        keystrokeCount += 1
        NSLog("[DEBUG] Count incremented to \(keystrokeCount)/\(keystrokeThreshold)")
    ...
    private func triggerBreak() {
        NSLog("[BREAK] Starting 20-second break")
        isOnBreak = true
    ...
    private func endBreak() {
        NSLog("[BREAK] Break complete, resuming")
        isOnBreak = false
    ...
    private func notify(title: String, body: String) {
        NSLog("[BREAK] 🔔 Notification: \(title) - \(body)")
